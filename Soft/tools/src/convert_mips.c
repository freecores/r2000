...
#define RELEASE1
/*
   RELEASE1: 09-12-2006/18-11-1427
   Abdallah Meziti El-Ibrahimi
   abdallah.meziti@gmail.com
   generate more files format
   name_code.txt : 76543210		: code standard
   name_sraml.txt: 3210			: sram low
   name_sramh.txt: 7654			: sram high
   name_behav.txt: 76 54 32 10	: code behavioral
   usage convert_bin.exe name
 */
...
#ifdef RELEASE1
char	namefile[80];	/* write the object code to this file		*/
#endif
...
#ifdef RELEASE1
	{
		char filecode[80], filesraml[80], filesramh[80], filebehav[80];
		
		strcpy(filecode,namefile);strcat(filecode,"_code.txt");
		strcpy(filesraml,namefile);strcat(filesraml,"_sraml.txt");
		strcpy(filesramh,namefile);strcat(filesramh,"_sramh.txt");
		strcpy(filebehav,namefile);strcat(filebehav,"_behav.txt");
		
		codefile  = fopen(filecode,"w");
		sramlfile = fopen(filesraml,"w");
		sramhfile = fopen(filesramh,"w");
		behavfile = fopen(filebehav,"w");
			
		for(i = 0; i <= length; i += 4)
		{
			d = ntohl(*(unsigned long *)(code + i));
			/* generate the code output files */
			fprintf(codefile, "%8.8x\n", d);
			
			/* generate the sraml output files */
//			fprintf(sramlfile, "%4.4x\n", (unsigned int)(d));
			fprintf(sramlfile, "%2.2x", (unsigned char)(d>>8));
			fprintf(sramlfile, "%2.2x\n", (unsigned char)(d));
			
			/* generate the sramh output files */
			fprintf(sramhfile, "%4.4x\n", (unsigned int)(d>>16));
			
			/* generate the behavioral output files */
			fprintf(behavfile, "%2.2x ", (unsigned char)(d>>24));
			fprintf(behavfile, "%2.2x ", (unsigned char)(d>>16));
			fprintf(behavfile, "%2.2x ", (unsigned char)(d>>8));
			fprintf(behavfile, "%2.2x\n", (unsigned char)(d));
		}
